...
def run_mcmc(lkf,iterations=3000,outname='chain.npy'):
...
def analyse_mcmc(lkf,chain,burnin=400,do_plot=False,do_print=False,omit_width=True,plot_chain=True,do_show=True):
    ...
    if name is not None:
        ...
        prefix=''
    labels=('Inclination angle $i$','Cone angle $\\psi$','Phase $\\theta$','$\log_{10}(p/{\\rm Myr})$','$v$','Pos angle $\\alpha$', 'line width $V$')
